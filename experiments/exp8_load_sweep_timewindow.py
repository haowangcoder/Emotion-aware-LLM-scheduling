#!/usr/bin/env python3
"""
Exp-8: Load Sweep (Time Window Mode)

Purpose:
- Same as Exp-2, but using time_window mode instead of fixed_jobs mode
- Demonstrate that results hold across different load levels with throughput focus
- Show how AW-SSJF behaves from low load (rho=0.5) to saturation (rho>=1.0)
- Compare SJF, AW-SSJF(k=2), AW-SSJF(k=4) at each load level

Parameters:
- system_load (rho) in {0.5, 0.7, 0.9, 1.0, 1.2}
- Schedulers: SJF, AW-SSJF(k=2), AW-SSJF(k=4)
- Mode: time_window (fixed duration, measure throughput)

Key insights:
- rho=0.5: Low load, scheduling differences minimal
- rho=0.9: Current experiment point, differences visible
- rho>=1.0: System saturation, AW-SSJF differences amplified
- Time window mode captures throughput-focused metrics
...
Usage:
    ...
    # Then analyze:
    python experiments/exp8_load_sweep_timewindow.py \
        --input_dir results/experiments/exp8_load_sweep_timewindow \
        --output_dir results/experiments/exp8_load_sweep_timewindow/plots
"""
    ...
    for load, schedulers in sorted(results.items()):
        ...
        for sched, summary in schedulers.items():
            ...
            if metric == 'depression_wait':
                ...
                # Extract throughput from run_metrics or calculate
                value = run_metrics.get('throughput', overall_metrics.get('throughput'))
            elif metric == 'jobs_completed':
                value = run_metrics.get('jobs_completed', overall_metrics.get('jobs_completed'))
            else:
...
def plot_performance_vs_load(
    ...
    # Mark saturation point at rho=1.0
    ...
    ax.set_xlabel('System Load (rho)', fontsize=12)
...
def plot_throughput_vs_load(
    ...
    """
    Plot throughput vs load for all schedulers (time_window specific).
    ...
    plot_performance_vs_load(
        results, output_path,
        metric='throughput',
        ylabel='Throughput (jobs/second)',
        title='Throughput vs Load (Time Window Mode)\n(Higher is better)'
    )


def plot_jobs_completed_vs_load(
    results: Dict[float, Dict[str, Dict]],
    output_path: Path
) -> None:
    """
    Plot jobs completed vs load for all schedulers (time_window specific).
    ...
    plot_performance_vs_load(
        ...
        title='Jobs Completed vs Load (Time Window Mode)'
    )
...
def plot_depression_vs_load(
    ...
    plot_performance_vs_load(
        ...
        title='Depression User Wait Time vs Load (Time Window Mode)\n(Lower is better for fairness)'
    )
...
def plot_tail_latency_vs_load(
    ...
    """
    Plot P99 tail latency vs load (using adjusted metrics that include pending jobs).
    ...
    # Use adjusted metrics that include pending jobs (fixes survivor bias)
    df_p95 = extract_load_sweep_metrics(results, 'adjusted_p95_waiting_time')
    df_p99 = extract_load_sweep_metrics(results, 'adjusted_p99_waiting_time')

    # Fallback to original metrics if adjusted not available
    if df_p99.isna().all().all():
    ...
    ax.set_title('Tail Latency (P99) vs Load (Time Window Mode)\n(Solid=P99, Dashed=P95, includes pending jobs)', fontsize=14)
    ax.set_xticks(loads)
    ax.legend(loc='upper left')
    ax.grid(True, alpha=0.3)
...
def plot_speedup_vs_load(
    ...
    ax.set_title('Depression User Speedup vs Load (Time Window Mode)\n(Higher is better)', fontsize=14)
    ax.set_xticks(loads)
    ax.legend(loc='upper left')
    ax.grid(True, alpha=0.3)
...
def generate_load_sweep_report(
    ...
    """
    report = {
        'experiment': 'exp8_load_sweep_timewindow',
        'purpose': 'Demonstrate results across different load levels (time_window mode)',
        'mode': 'time_window',
        'parameters': {
    ...
    for load, schedulers in sorted(results.items()):
        ...
        for sched, summary in schedulers.items():
            ...
            report['results_by_load'][str(load)][sched] = {
                ...
                'throughput': run_metrics.get('throughput', overall_metrics.get('throughput')),
                'jobs_completed': run_metrics.get('jobs_completed', overall_metrics.get('jobs_completed')),
...
def main():
    parser = argparse.ArgumentParser(
        description="Exp-8: Load Sweep Analysis (Time Window Mode)"
    )
    parser.add_argument(
        "--input_dir",
        type=str,
    ...
    for load, scheds in sorted(results.items()):
        print(f"  rho={load}: {list(scheds.keys())}")
    ...
    plot_throughput_vs_load(results, output_dir / "throughput_vs_load.png")
    plot_jobs_completed_vs_load(results, output_dir / "jobs_completed_vs_load.png")

    # Generate report
    print("\n=== Generating Report ===")
    report = generate_load_sweep_report(results, output_dir / "exp8_report.json")
